...
import { STATIC_CATEGORIES, searchAPI, shopAPI } from '../services/api';
...
const StoreFront: React.FC<StoreFrontProps> = ({ sidebarOpen, setSidebarOpen }) => {
    ...
    const fetchProducts = async () => {
      ...
      try {
        if (activeShopId) {
          ...
          const shopData = await shopAPI.getShopBySlug(activeShopId);
          setProducts(shopData.inventory);
        } else {
          // For initial load, fetch products using a general search
          const allProducts = await searchAPI.search('', 100, 0);
  ...
  const selectedShop = activeShopId ? shops.find(s => s.slug === activeShopId) : null;
  ...
  return (
    <div className="flex max-w-7xl mx-auto items-start gap-6 pt-4 px-0 md:px-4">
      ...
      <main className="flex-1 min-w-0">
        ...
        {/* Error State */}
        {error && !loading && (
          <div className="bg-red-50 border border-red-200 rounded-lg p-4 mx-4 md:mx-0 mb-4">
